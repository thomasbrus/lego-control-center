...
export const Indicator = forwardRef<HTMLSpanElement, IndicatorProps>(function Indicator(props, ref) {
...
export const ThumbIndicator = forwardRef<HTMLSpanElement, ThumbIndicatorProps>(function SwitchThumbIndicator(props, ref) {
  const { fallback, children, ...rest } = props;
  const api = useSwitchContext();
  return (
...
export type DefaultProps = RootProps & { label: string };

export function Default({ label, ...props }: DefaultProps) {
  return (
    <Root {...props}>
      <Control>
        <Thumb />
