# region imports
from pybricks.hubs import hub, version
from pybricks.pupdevices import Motor, ColorDistanceSensor
from pybricks.parameters import Port, Color
from pybricks.tools import wait, multitask, run_task
from pybricks.iodevices import AppData
...
# Combines and runs all tasks in parallel.
run_tasks = lambda: run_task((multitask(*tasks)))
...
stdin_command_handlers[0x30] = lambda args: hub.light.on(Color(*args[0:3]))
stdin_command_handlers[0x31] = lambda args: hub.light.off()
# endregion
...
async def broadcast_motor_states_loop():
    while True:
        enumerate_devices(Motor, broadcast_motor_state)
        await wait(100)
...
async def broadcast_cd_sensor_states_loop():
    while True:
        await enumerate_devices(ColorDistanceSensor, broadcast_cd_sensor_state)
        await wait(100)
