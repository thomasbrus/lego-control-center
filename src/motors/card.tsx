import { Badge, Button, Icon, Input, InputGroup, Progress, PropertyList, Slider, Text } from "@/components";
import { CableIcon, CircleGaugeIcon, GaugeIcon, ZapIcon } from "lucide-react";
import { useState } from "react";
import { Flex, Grid, styled } from "styled-system/jsx";
import { getDevicePortName } from "@/devices/device";
import { Motor, MotorLimits, MotorState } from "./motor";
import { DeviceCard, DeviceDetailsSection } from "@/devices/card";

export function MotorCard({ port, motor }: { port: number; motor: Motor }) {
  return (
    <DeviceCard
      title={`Motor Port ${getDevicePortName(port)}`}
      icon={<CableIcon />}
